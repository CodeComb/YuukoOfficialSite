...
function LoadFromCollectionPort(port) {
    ...
    $.getJSON("/yuuko/gets/" + port, Collection[port], function (data) {
        ...
        $("[data-collection='" + port + "']").unbind().each(function () {
            var template = $(this).find('.template.' + port.toLocaleLowerCase());
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var x in data[i]) {
                    if (typeof (x) == "undefined") continue;
                    if (NewDom.find("[data-field='" + port + "." + x + "']").val() == null) {
                    ...
                    if (NewDom.attr("data-field") == port + "." + x) {
                        if (NewDom.val() == null) {
                            try { NewDom.val(data[i][x].toString()); } catch (e) { }
                        }
                        if (NewDom.attr("data-collection") == null && NewDom.attr("data-detail") == null) {
                            try { NewDom.text(data[i][x].toString()); } catch (e) { }
                            try { NewDom.html(data[i][x].toString()); } catch (e) { }
                        }
                    }
                }
                $(this).append(NewDom);
        ...
        $("[data-detail='" + port + "']").unbind().each(function () {
            if ($(this).attr("data-method") == null || $(this).attr("data-method") != "insert") {
                ...
                for (var x in data) {
                    ...
                    if ($(this).attr("data-collection") == null && $(this).attr("data-detail") == null) {
